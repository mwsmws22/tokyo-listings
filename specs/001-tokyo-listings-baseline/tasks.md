...
- [X] T059 Extend `docker/docker-compose.yml` (or root `compose.yaml`) with **`api`** and **`web`** services, shared env / `env_file`, `depends_on` **`postgres`** with **`condition: service_healthy`**, published ports (align API **4001**, web **3000** with `plan.md` / quickstart), and networking so the browser hits the **web** service while API is reachable for health checks and (if needed) direct debugging
- [X] T064 [P] Replace stub **`docker/Dockerfile.api`** with a multi-stage image that builds/installs the monorepo workspace and runs the API entrypoint (`apps/api`); ensure `DATABASE_URL` and auth env are overridable at runtime
- [X] T065 [P] Replace stub **`docker/Dockerfile.web`** with a multi-stage image that builds Next (`apps/web` standalone output per Next docs), including **PostCSS** (Tailwind v4 + Uniwind) so `globals.css` and vendored Rosé Pine CSS compile in CI; run with correct **`NEXT_PUBLIC_*`** / server env for API rewrites or internal service URL inside the compose network
- [X] T066 Add **`docker/README.md`** (or a dedicated section in `quickstart.md` if preferred) documenting: `docker compose up --build`, required env files for Compose, how **`BETTER_AUTH_URL`** matches the **published web URL**, and a short smoke checklist (health + auth)
...
- [X] T036 [US2] Implement `apps/web/src/components/MapShell.tsx` loading Maps JavaScript API using `GOOGLE_MAPS_API_KEY_CLIENT` (via `next.config.ts` `env`) and rendering a full-height map
