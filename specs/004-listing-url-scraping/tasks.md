...
**Goal**: Use list-row selection (no checkbox) styled consistently with existing property/list rows (rose-pine border/highlight). Selecting a property means submit associates the new listing to that property, and the newly created listing appears in the right-hand “Listings” section for that selected property.

**Independent test**: On add page, select a property row from similar-properties list (no checkbox/toggle) and submit; created listing is linked to that property and appears in the right-hand property listings panel. If no property is selected, submit keeps default behavior (create new property). For linked saves, non-empty property address/type fields remain immutable and only missing parts are fillable.

- [X] T050 [US3] Extend create payload contract in `/home/smbuser/mws-server/tokyo-listings/packages/validators/src/listing.ts` and `/home/smbuser/mws-server/tokyo-listings/packages/validators/src/scraping.ts` with optional `selectedPropertyId` (selection itself is the association signal; no confirm checkbox/toggle flag)
- [X] T051 [US3] Update add-listing selection UX in `/home/smbuser/mws-server/tokyo-listings/apps/web/src/components/listing/SimilarPropertiesPicker.tsx` and `/home/smbuser/mws-server/tokyo-listings/apps/web/src/components/listing/ListingFormParity.tsx` to use row selection styling consistent with other property/list rows (rose-pine border/highlight) and persist selected property id in submit state
- [X] T052 [US3] Implement `listing.create` association branch in `/home/smbuser/mws-server/tokyo-listings/apps/api/src/trpc/routers/listing.ts`: if `selectedPropertyId` is present, attach listing to that property; otherwise create new property as before
- [X] T053 [US3] Enforce property-field immutability rules in `/home/smbuser/mws-server/tokyo-listings/apps/api/src/trpc/routers/listing.ts` and helper under `/home/smbuser/mws-server/tokyo-listings/apps/api/src/lib/`: property type cannot be modified when linking; address parts cannot overwrite non-empty stored values; allow filling only missing address parts (e.g. unknown house number)
- [X] T054 [US3] Lock/disallow editing property-bound fields in add UI when a property row is selected for association in `/home/smbuser/mws-server/tokyo-listings/apps/web/src/components/listing/ListingFormParity.tsx` (keep listing fields editable: rent/reikin/shikikin/area/station/walk/availability)
- [X] T055 [P] [US3] Add API/UI integration coverage for selected-property save behavior and missing-address-part merge logic in `/home/smbuser/mws-server/tokyo-listings/apps/api/test/trpc/routers/listing.preview.integration.test.ts` (or new `listing.create.association.integration.test.ts`) and `/home/smbuser/mws-server/tokyo-listings/apps/web/src/components/listing/` ensuring the saved listing shows under the selected property’s right-panel “Listings”
