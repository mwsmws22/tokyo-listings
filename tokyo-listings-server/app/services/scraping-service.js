...
class Parser {
  ...
  async parseAtHome(html) {
    ...
    let reikin = $('th:contains("礼金") + td', '.mainItemInfo.bukkenOverviewInfo')
      .text()
      .replace('ヶ月', '')
    if (reikin === 'なし') {
    ...
    if (security_deposit === 'なし') {
      ...
      output.listing.security_deposit = Number(
        (
          parseFloat(security_deposit.replace('万円', '')) /
  ...
  async parseSuumo(html) {
    ...
    let reikin = $('span:contains("礼金")', '.property_view_note-list')
      .text()
      .replace('礼金: ', '')
      .replace('万円', '')
    if (reikin === '-') {
    ...
    let security_deposit = $('span:contains("敷金")', '.property_view_note-list')
      .text()
      .replace('敷金: ', '')
      .replace('万円', '')
    if (security_deposit === '-') {
      ...
      output.listing.security_deposit = Number(
        (parseFloat(security_deposit) / parseFloat(output.listing.monthly_rent)).toFixed(
          1
        )
      ).toString()
    }
  ...
  async parseYahoo(html) {
    ...
    let reikin = $('dt:contains("礼金：") + dd').text().remove('万円', ' ※')
    if (reikin === 'なし') {
      output.listing.reikin = '0'
    ...
    let security_deposit = $('dt:contains("敷金：") + dd')
      .text()
      .replace('万円', '')
      .replace(' ※', '')
    if (security_deposit === 'なし') {
    ...
    stations.each(function (i, elem) {
      ...
      if (text.indexOf('バス') === -1) {
        let tempStation = text.match(/(.*?)駅/i)[1].noSpaces() + '駅'
  ...
  async parseRealTokyoEstate(html) {
    ...
    let security_deposit = $('span', '#estate_info_deposite_value')
      .text()
      .replace('ヶ月', '')
    if (security_deposit.includes('円')) {
      output.listing.security_deposit =
        parseInt(security_deposit.replace(',', '').replace('円')) /
        10000 /
        output.listing.monthly_rent
    ...
    let stationMatches = $('td:contains("交通：") + td')
      .text()
      .matchAll(/「(.*?)分/g)
  ...
  async parseTokyoStyle(html) {
    ...
    output.listing.closest_station =
      boxText.match(/(?:線|ライナー)\s(.*?)[\s,駅,歩,徒]/)[1] + '駅'
  ...
  async parseAerasGroup(html) {
    ...
    let property_type = $('dt:contains("種別/構造：") + dd')
      .text()
      .match(/賃貸(.*?)\//)[1]
  ...
  async parseRehouse(html) {
    ...
    let property_type = $('.mrh-label-article')
      .text()
      .match(/賃貸(.*?)$/)[1]
    ...
    let stationMatches = $('th:contains("最寄り駅") + td')
      .text()
      .matchAll(/「(.*?)分/g)
  ...
  async parseChintai(html) {
    ...
    let security_deposit = $('th:contains("敷金") + td')
      .text()
      .match(/(.*?)\//)[1]
      .replace(/\s/g, '')
    let reikin = $('th:contains("礼金") + td')
      .text()
      .match(/(.*?)\//)[1]
      .replace(/\s/g, '')
    ...
    if (security_deposit.includes('ヶ月')) {
      ...
      output.listing.security_deposit =
        parseFloat(security_deposit.replace('万円', '')) / output.listing.monthly_rent
    }
  ...
  async parseOmusubiEstate(html) {
    ...
    let stationMatches = $('dt:contains("交通") + dd')
      .text()
      .matchAll(/線(.*?)分/g)
  ...
  async parseRStore(html) {
    ...
    let stationMatches = $('h3:contains("最寄り駅") + p', 'li')
      .text()
      .matchAll(/「(.*?)分/g)
  ...
  async parseLifullHomes(html) {
    ...
    let security_deposit = $('#chk-bkc-moneyshikirei')
      .text()
      .match(/(.*?)\s\//)[1]
    let reikin = $('#chk-bkc-moneyshikirei')
      .text()
      .match(/\/\s(.*?)$/m)[1]
  ...
  async parseJoylifestyle(html) {
    ...
    if (security_deposit.includes('ヶ月')) {
      ...
      output.listing.security_deposit = Number(
        (
          parseFloat(security_deposit.replace('円', '')) /
          10000 /
          parseFloat(output.listing.monthly_rent)
        ).toFixed(2)
      ).toString()
    ...
    let stationMatches = $('th:contains("交通") + td')
      .text()
      .matchAll(/線(.*?)分/g)
  ...
  async parseDiyp(html) {
    ...
    let security_deposit_reikin = $('p:contains("敷金：")')
      .text()
      .noSpaces()
      .replace(/\([^()]*\)/g, '')
      .replace(/（[^（）]*）/, '')

    output.listing.security_deposit = security_deposit_reikin
      .match(/敷金：(.*?)礼金：/)[1]
      .replace('ヶ月', '')
      .replace('なし', '0')
      .replace('-', '0')
    output.listing.reikin = security_deposit_reikin
      .match(/礼金：(.*?)償却：/)[1]
      .replace('ヶ月', '')
      .replace('なし', '0')
      .replace('-', '0')
    ...
    let stationMatches = $('dt:contains("最寄駅：") + dd')
      .text()
      .matchAll(/「(.*?)分/g)
  ...
  async parsePetHomeWeb(html) {
    ...
    var security_deposit = $('th:contains("敷金/保証金") + td')
      .text()
      .replace(/(\r\n|\n|\r)/gm, '')
      .noSpaces()
      .match(/(.*?)\//)[1]
    var reikin = $('th:contains("礼金") + td')
      .text()
      .match(/(.*?)(\/|$)/)[1]
      .replace('※税込', '')
  ...
  async parseTomigaya(html) {
    ...
    output.listing.availability = '募集中'
    output.listing.monthly_rent =
      parseInt(monthly_rent.replace('円', '').replace(',', '')) / 10000
    output.listing.square_m = $('th:contains("専有面積") + td')
      .text()
      .replace('m²', '')
      .noSpaces()
  ...
  async parseJKHome(html) {
    ...
    let stationMatches = $('th:contains("アクセス") + td')
      .text()
      .matchAll(/[【｢『「\s](.*?)分/g)
  ...
  async parseSpacelist(html) {
    ...
    let property_type = $('li:contains("建物種類")')
      .text()
      .match(/：(.*?)$/)[1]
  ...
  async parseSumaity(html) {
    ...
    let security_deposit = $('th:contains("敷金/礼金") + td')
      .text()
      .match(/^(.*?)\//)[1]
    let reikin = $('th:contains("敷金/礼金") + td')
      .text()
      .match(/\/(.*?)$/)[1]
  ...
  async parseKenCorp(html) {
    ...
    let stationMatches = $('th:contains("最寄駅") + td')
      .text()
      .matchAll(/線(.*?)分/g)
  ...
  async parseSmocca(html) {
    ...
    let property_type = $('th:contains("種別/構造") + td')
      .text()
      .match(/^(.*?)\//)[1]
    ...
    let stationMatches = $('th:contains("主要交通機関") + td')
      .text()
      .matchAll(/\/(.*?)分/g)
  ...
  async parseAxelHome(html) {
    ...
    let stationMatch = $('th:contains("交通") + td')
      .text()
      .match(/線(.*?)分/)[1]
  ...
  async parseOasisEstate(html) {
    ...
    let stationMatches = $('th:contains("交通アクセス") + td')
      .text()
      .matchAll(/\/(.*?)分/g)
