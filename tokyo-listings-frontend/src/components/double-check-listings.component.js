...
class DoubleCheckListings extends React.Component {
  constructor(props) {
    super();
    this.getListings = this.getListings.bind(this);
    this.checkURLs = this.checkURLs.bind(this);
    this.scrapeCheck = this.scrapeCheck.bind(this);
    this.setListingToAvailable = this.setListingToAvailable.bind(this);
  ...
  setListingToAvailable(l) {
    ListingDataService.update(l.id, {availability: "募集中"})
      .then(response => {
  ...
  render() {
    return (
      <div style={{ marginLeft: 10, marginRight: 10 }}>
        <h4 className="text-center mb-0" style={{paddingTop: 18, paddingBottom: 22}}>Double Check Listings</h4>
            ...
            {this.state.listings.map(l =>
              <ListGroup.Item key={l.id} onClick={() => this.selectProperty(l)} variant={l.id === this.state.selectedListingID && "dark"}>
                <div className="row justify-content-between" style={{height: this.state.list_item_height-25}}>
                  <div className="col-9">
                    <a href={l.url} target="_blank" color={ this.state.selectedProperty?.id === l.property_id ? "green" : undefined } style={{display: "block", textOverflow: "ellipsis", overflow: "hidden", maxWidth: "100%"}}>
                      <span style={{ whiteSpace: "nowrap" }}>{ l.url }</span>
                    </a>
                  </div>
                  <div className="text-right" style={{paddingRight: 20}} onClick={() => this.setListingToAvailable(l)}>
                    <Badge pill variant="success" style={{fontSize: "0.8rem", paddingBottom: "4px", cursor:"pointer"}}> 募集中 </Badge>
                  </div>
                </div>
              </ListGroup.Item>
            )}
...
export default DoubleCheckListings;
